use crate::common::*;
use anyhow::Result;

#[test]
fn menclose_actuarial() -> Result<()> {
    let expr = "<math>
                    <menclose notation='actuarial'>  <mn>3</mn><mo>+</mo><mn>2</mn><mi>i</mi> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "simbolo attuariale, racchiude 3 più 2 i fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_box() -> Result<()> {
    let expr = "<math>
                    <menclose notation='box circle'>  <mn>3</mn><mo>+</mo><mn>2</mn><mi>i</mi> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "riquadro, cerchio, racchiude 3 più 2 i fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_left() -> Result<()> {
    let expr = "<math>
                    <menclose notation='left'>  <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "riga a sinistra, racchiude tre mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_right() -> Result<()> {
    let expr = "<math>
                    <menclose notation='right'>  <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "riga a destra, racchiude tre mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_top_bottom() -> Result<()> {
    let expr = "<math>
                    <menclose notation='top bottom'>  <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "riga sopra, sotto, racchiude 3 mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_updiagonalstrike() -> Result<()> {
    let expr = "<math>
                    <menclose notation='updiagonalstrike'>  <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "diagonale verso l'alto, barrata, racchiude 3 mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_downdiagonalstrike() -> Result<()> {
    let expr = "<math>
                    <menclose notation='downdiagonalstrike'>  <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "diagonale verso il basso, barrata, racchiude tre mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_cross_out() -> Result<()> {
    let expr = "<math>
                    <menclose notation='updiagonalstrike downdiagonalstrike'>  <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "x, barrata, racchiude 3 mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_vertical_horizontal_strike() -> Result<()> {
    let expr = "<math>
                    <menclose notation='verticalstrike horizontalstrike'>  <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "verticale, orizzontale, barrata, racchiude 3 mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_leftarrow() -> Result<()> {
    let expr = "<math>
                    <menclose notation='leftarrow'> <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "freccia a sinistra, racchiude tre mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_right_up_down_arrow() -> Result<()> {
    let expr = "<math>
                    <menclose notation=' rightarrow downarrow  uparrow  '> <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "freccia in alto, freccia in basso, freccia a destra, racchiude tre mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_northeastarrow() -> Result<()> {
    let expr = "<math>
                    <menclose notation='northeastarrow'> <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "freccia a nordest, racchiude 3 mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_other_single_arrows() -> Result<()> {
    let expr = "<math>
                    <menclose notation='northwestarrow southwestarrow southeastarrow'> <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "freccia a sudest, freccia a sudovest, freccia a nordovest, racchiude 3 mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_northwestsoutheastarrow() -> Result<()> {
    let expr = "<math>
                    <menclose notation='northwestsoutheastarrow'> <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "freccia diagonale in basso a due punte, racchiude 3 mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_other_double_arrows() -> Result<()> {
    let expr = "<math>
                    <menclose notation='updownarrow leftrightarrow northeastsouthwestarrow'>  <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "freccia verticale a due punte, freccia orizzontale a due punte, freccia diagonale in alto a due punte, racchiude 3 mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_madrub() -> Result<()> {
    let expr = "<math>
                    <menclose notation='madrub'> <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "simbolo fattoriale arabo, racchiude 3 mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_phasorangle() -> Result<()> {
    let expr = "<math>
                    <menclose notation='phasorangle'> <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "angolo di fase, racchiude 3 mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_circle_phasorangle() -> Result<()> {
    let expr = "<math>
                    <menclose notation='phasorangle circle'> <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "cerchio, angolo di fase, racchiude 3 mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_longdiv() -> Result<()> {
    let expr = "<math>
                    <menclose notation='longdiv'> <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "simbolo di divisione lunga, racchiude tre mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_longdiv_default() -> Result<()> {
    let expr = "<math>
                    <menclose> <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "simbolo di divisione lunga, racchiude 3 mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_longdiv_empty_string() -> Result<()> {
    let expr = "<math>
                    <menclose notation=''> <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "simbolo di divisione lunga, racchiude 3 mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_longdiv_whitespace_string() -> Result<()> {
    let expr = "<math>
                    <menclose notation='  '> <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "simbolo di divisione lunga, racchiude tre mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn menclose_radical() -> Result<()> {
    let expr = "<math>
                    <menclose notation='radical'> <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "radice quadrata, racchiude tre mezzi fine contenuto racchiuso")?;
    return Ok(());

}

#[test]
fn simple_speak_menclose_top_bottom() -> Result<()> {
    let expr = "<math>
                    <menclose notation='top bottom'>  <mfrac><mn>3</mn><mn>2</mn></mfrac> </menclose>
                </math>";
    test("it", "ClearSpeak", expr, "riga sopra, sotto, racchiude tre mezzi fine contenuto racchiuso")?;
    return Ok(());

}
