...
fn modified_vars() -> Result<()> {
    ...
    test("it", "SimpleSpeak", expr, 
        "a grave, b tilde, c breve, b caron, c accento grave; più \
            r caron più; x punto, y punto sopra, z dieresi, u tre puntini sopra
		, v quadruplo punto; più  x chiocciola, più vettore t")?;
            return Ok(());
...
fn limit() -> Result<()> {
    ...
    test("it", "SimpleSpeak", expr, "limite per x tendente a 0, di, frazione, seno di x, fratto x, fine frazione")?;
    test_prefs("it", "SimpleSpeak", vec![("Impairment", "LearningDisability")], expr,
            "limite per x tendente a 0, di; seno di x, fratto x")?;
            return Ok(());
...
fn limit_from_below() -> Result<()> {
    ...
    test("it", "SimpleSpeak", expr, "limite per x tendente dal basso a 0, di seno di x")?;
    return Ok(());
...
fn binomial_mmultiscripts() -> Result<()> {
    ...
    test("it", "SimpleSpeak", expr, "n su m")?;
    return Ok(());
...
fn permutation_mmultiscripts() -> Result<()> {
    ...
    test("it", "SimpleSpeak", expr, "k permutazioni di n")?;
    return Ok(());
...
fn tensor_mmultiscripts() -> Result<()> {
    ...
    test_prefs("it", "SimpleSpeak", vec![("Verbosity", "Verbose")], expr,
            "r con 4 indici a destra, subscript i superscript j subscript k subscript l")?;
    test_prefs("it", "SimpleSpeak", vec![("Verbosity", "Medium")], expr,
            "r con 4 indici a destra, sub i super j sub k sub l")?;
            return Ok(());
...
fn huge_num_mmultiscripts() -> Result<()> {
    ...
    test_prefs("it", "SimpleSpeak", vec![("Verbosity", "Verbose")], expr,
            "r con 4 indici a sinistra, pre subscript i, pre superscript j e indici alternati, cap k none cap l none end prescripts and with 5 postscripts, subscript i superscript j subscript k subscript l and alternating scripts m none end scripts")?;
            return Ok(());
...
fn prime() -> Result<()> {
    ...
    test("it", "SimpleSpeak", expr, "x primo")?;
    return Ok(());
...
fn given() -> Result<()> {
    ...
    test("it", "SimpleSpeak", expr, "P, aperta parentesi, a dato b, chiusa parentesi")?;
